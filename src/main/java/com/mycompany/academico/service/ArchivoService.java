...
public class ArchivoService {
   ...
   public void guardarCSV(List<Estudiante> estudiantes, File archivo) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(
            ...
            // Encabezado
            writer.write("id,nombre,edad,nota1,nota2,nota3");
            writer.newLine();

            for (Estudiante est : estudiantes) {
                  
                // Usamos Locale.US para forzar punto decimal (.) en los números  
                String linea = String.format(Locale.US, "%s,\"%s\",%d,%.2f,%.2f,%.2f",
                        est.getId(),
                        est.getNombre(),
                        est.getEdad(),
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                if (datos.length == 6) {
                    try {
                        ...
                        notas.add(new Nota(Double.parseDouble(datos[3])));
                        ...
                        // Manejar errores de formato si los hay
